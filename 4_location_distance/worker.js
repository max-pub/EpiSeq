...
export function start(locationList) {
	console.log('start contact calculation worker')
	let locationTree = groupByPatientID(locationList)
...
export function groupByPatientID(list) {
	...
	for (let [id, row] of Object.entries(list)) {
		...
		row.from = Date.parse(row.from.replaceAll(' ', 'T')) / 1000
		row.till = Date.parse(row.till.replaceAll(' ', 'T')) / 1000
		tree[row.patientID] ??= {}
