...
export function one(title, table) {
    ...
    $('output').insertAdjacentHTML('beforeend', `<h2 id='${id}'>${title}</h2>`)
    $('output').insertAdjacentHTML('beforeend', `<div id='${id}'>${table}</div>`)
...
export function two(title, table1, table2) {
    ...
    $('output').insertAdjacentHTML('beforeend', `<h2 id='${id}'>${title}</h2>`)
    if (subtitle)
        $('output').insertAdjacentHTML('beforeend', `<h3 id='${id}'>${subtitle}</h3>`)
    $('output').insertAdjacentHTML('beforeend', `<div id='${id}' class='flex-row'><div>${table1}</div><div>${table2}</div></div>`)
}
...
//     if (Array.isArray(title)) [title, subtitle] = title
//     let id = title.toLowerCase().replace(/[^a-z]/gi, '_');
//     $('#output').insertAdjacentHTML('beforeend', `<tr id='${id}' class='title'> <td colspan="2" class='title'>${title}</td> </tr>`)
//     if (subtitle)
