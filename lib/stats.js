...
export function locationListStats(LIST) {
	...
	for (let type of types) {
		for (let row in LIST) {
			...
			if (!value) continue
			output[type][value] ??= { count: 0 }
			output[type][value].count++
		}
	}
	return output
}

export function groupDistanceMatrixByDistance(dist, schema) {
	let out = {}
	...
	// console.log('start pairs')
	for (let id1 in dist) {
		// out[pid1] = {}
		for (let id2 in dist[id1]) {
			if (id1 <= id2) continue
			// console.log(pid1,pid2,data[pid1][pid2])
			let distance = String(dist[id1][id2])?.split('|')?.[0] * 1
			// if (distance === 2) console.log('pairs2', distance, id1, id2)
	...
	if (schemaLength)
		for (let distance = 0; distance <= schemaLength; distance++)
			out[distance] ??= []
	// console.log('size', Object.keys(out).length)
	// console.log('pairs 22',out[2])
	return out
}



export function getPatientAndEntryCounts(list) {
	return {
...
export function groupDistanceMatrixByPatientID(matrix, metaData) {//, first = false) {
	...
	for (let sid1 of sampleIDs) {
		...
		postMessage(['progress', sampleIDs.indexOf(sid1) + 1, sampleIDs.length])
