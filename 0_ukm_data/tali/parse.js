...
async function outputStream(lineStream, key, type) {
	...
	// Thread.post.done(key, type)
}

async function outputString(lineStream) {
	let out = {}
	for await (let [table, id, dict, total] of taliStream(lineStream, {})) {
...
export async function parse(string, options = {}) {
	let stream = lineStreamFromText(string)
	return await outputString(stream)
}