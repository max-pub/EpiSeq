...
export class DistanceMatrix extends Matrix {
    ...
    set(k1, k2, v) {
        ...
        [k1, k2] = this.ord(k1, k2)
        if (Array.isArray(v)) {
            v = v.map(val => this.testValue(val, k1, k2)).filter(x => x != undefined) // remove undefined values
        } else {
            v = this.testValue(v, k1, k2)
        }
        this.data[k1] ??= {}
        this.data[k1][k2] = v
        this.data[k2] ??= {} // makes sure all keys exist in the row-headers
    ...
    *iterate({ skipEmpty = true, onProgress } = {}) {
        for (let [k1, k2, val, index, size] of this) {
            if (skipEmpty && !(Number.isFinite(val) || Array.isArray(val))) continue
